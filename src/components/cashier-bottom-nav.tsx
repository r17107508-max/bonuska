...
export function CashierBottomNav() {
  ...
  return (
    <>
      <nav
        className="fixed inset-x-0 bottom-0 z-50 border-t border-amber-100 bg-[#fffdf8]/95 px-3 pt-2 pb-[calc(0.75rem+env(safe-area-inset-bottom))] shadow-[0_-16px_40px_rgba(92,53,33,0.14)] backdrop-blur"
      ...
      >
        <div className="mx-auto grid max-w-xl grid-cols-3 items-end gap-2">
          ...
          <button
            ...
            className="flex min-h-14 w-full flex-col items-center justify-center gap-1 rounded-xl px-3 text-xs font-semibold text-[#7b6a5b] transition hover:bg-amber-50 hover:text-[#2f1d13] active:scale-95"
          >
      ...
      {isLogoutOpen && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center overflow-y-auto bg-[#2f1d13]/45 p-4 pt-[calc(1rem+env(safe-area-inset-top))] pb-[calc(1rem+env(safe-area-inset-bottom))]">
          <div
            ...
            className="flex w-full max-w-sm flex-col overflow-hidden rounded-xl bg-[#fffdf8] shadow-2xl"
          ...
          >
            ...
            <div className="grid shrink-0 grid-cols-2 gap-3 border-t border-slate-100 bg-white p-5">
              <button
                ...
                className="min-h-12 rounded-lg border border-amber-200 bg-white px-4 font-semibold text-[#5c3521] transition hover:bg-amber-50"
              >
                Отмена
              </button>
              <form action={logout}>
                <button type="submit" className="min-h-12 w-full rounded-lg bg-green-700 px-4 font-semibold text-white transition hover:bg-green-800">
                  Выйти
                </button>
              </form>
            </div>
