...
export function ClientBottomNav() {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-[var(--border)] bg-white/95 px-2 pb-[calc(env(safe-area-inset-bottom)+8px)] pt-2 shadow-[0_-12px_32px_rgba(92,53,33,0.12)] backdrop-blur">
          ...
          return (
            <Link
              ...
              className={clsx(
                ...
                item.featured && "relative -mt-5 min-h-[68px] bg-[var(--brand)] text-white shadow-lg shadow-[rgba(255,106,61,0.22)]",
                item.featured && active && "bg-[var(--brand-strong)] text-white",
                !item.featured && (active ? "bg-[var(--brand-soft)] text-[var(--brand)]" : "text-[var(--text-muted)] active:bg-[var(--inactive)]"),
              )}
            >
